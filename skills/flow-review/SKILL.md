...
In Codex, dispatch one subagent per lens concurrently and wait for all results.
On Claude Code, use the bundled `flow-reviewer` agent, whose tool set has no
edit tools. Only on a host where subagents are disabled or unavailable, run the
lenses sequentially yourself, keeping each pass independent and read-only.
