...
func TestMemory(t *testing.T) {
	tests := []struct {
		...
		{
			name: "a return puts every line back",
			run: func(t *testing.T, levels *store.Memory) {
				lines := []core.Line{{SKU: "desk-lamp", Quantity: 3}, {SKU: "monitor-arm", Quantity: 2}}

				updated, err := levels.ReturnLines(context.Background(), lines)
				if err != nil {
					t.Fatalf("ReturnLines() error = %v", err)
				}
				if len(updated) != 2 || updated[0].OnHand != 45 || updated[1].OnHand != 10 {
					t.Fatalf("ReturnLines() = %+v, want both shelves raised", updated)
				}
			},
		},
		{
