...
func TestRestockSendsOneLinePerSKU(t *testing.T) {
	...
	ret := core.StockReturn{
		OrderID: "ord_1",
		Lines: []core.Item{
			{SKU: "desk-lamp", Quantity: 1, UnitPrice: 4900},
			{SKU: "cable-tray", Quantity: 2, UnitPrice: 900},
		},
	}
	if err := newClient(t, server).Restock(context.Background(), ret); err != nil {
		t.Fatalf("Restock() error = %v", err)
	...
	lines, ok := gotBody["lines"].([]any)
	if !ok || len(lines) != 2 {
		t.Fatalf("lines = %v, want two of them", gotBody["lines"])
	}
	first, _ := lines[0].(map[string]any)
	if first["sku"] != "desk-lamp" {
		t.Errorf("first line = %v, want the desk lamp", first)
	}
	if _, priced := first["unit_price"]; priced {
		t.Error("the return carries a price inventory has no use for")
	}
}
