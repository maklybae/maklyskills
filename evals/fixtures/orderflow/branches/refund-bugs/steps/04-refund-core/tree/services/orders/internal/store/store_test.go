...
func TestRefundsAreStoredWithTheOrder(t *testing.T) {
	path := filepath.Join(t.TempDir(), "orders.json")
	orders, err := store.NewJSONFile(path)
	if err != nil {
		t.Fatalf("NewJSONFile() error = %v", err)
	...
	if len(stored.Refunds) != 1 || stored.Refunded() != 4900 {
		t.Fatalf("refunds = %+v, want the one that was written", stored.Refunds)
	}
}

func TestRefundedOrdersStayInTheListing(t *testing.T) {
	orders := newStore(t)
	...
	found, err := orders.List(context.Background(), core.Filter{CustomerID: "cust-17"})
	if err != nil {
		t.Fatalf("List() error = %v", err)
	}
	if len(found) != 1 {
		t.Fatalf("List() returned %d orders, want 1", len(found))
	}
	if found[0].Refunded() != 4900 {
		t.Fatalf("the listed order gave back %d, want 4900", found[0].Refunded())
	}
}
