...
func TestEndpoints(t *testing.T) {
	tests := []struct {
		...
		{
			name:       "refund with a body that is not json",
			method:     http.MethodPost,
			path:       "/orders/ord_delivered/refund",
			body:       `{"key":}`,
			wantStatus: http.StatusBadRequest,
		...
		{
			name:       "refund an order that is not there",
			method:     http.MethodPost,
			path:       "/orders/ord_missing/refund",
			body:       `{"key":"sup-4"}`,
			wantStatus: http.StatusNotFound,
			wantCode:   "order_not_found",
		},
...
func TestRefundViewNamesTheOrderAndItsLines(t *testing.T) {
	router, _ := newRouter(t)
	...
	if len(view.Lines) != 1 || view.Lines[0].SKU != "desk-lamp" || view.Lines[0].Quantity != 1 {
		t.Errorf("lines = %+v, want the line that came back", view.Lines)
	}
	if view.IssuedAt == "" {
		t.Error("the refund does not say when it was issued")
	}
}
