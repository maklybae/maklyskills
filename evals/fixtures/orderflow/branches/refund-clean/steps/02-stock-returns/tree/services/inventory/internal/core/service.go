...
// ReturnRequest puts the lines of a refunded order back on the shelf. The order
// id is there for the audit trail; the shelf is moved by the lines alone.
...
// Return books the lines of a refund back onto the shelf.
//
// A return is counted, not keyed: inventory has no way to tell a resend from a
// second parcel, so the caller has to send each return exactly once.
func (s *Service) Return(ctx context.Context, req ReturnRequest) ([]Level, error) {
	...
	levels, err := s.store.ReturnLines(ctx, req.Lines)
	if err != nil {
		return nil, xerrors.Wrapf(err, "take the return of order %s back", req.OrderID)
	}
	return levels, nil
}
...
func (r ReturnRequest) validate() error {
	...
	if len(r.Lines) == 0 {
		return invalid("lines", "a return needs at least one line")
	}
	return validateLines(r.Lines)
}

func validateLines(lines []Line) error {
	if len(lines) > maxLinesPerReservation {
		return invalid("lines", "at most "+strconv.Itoa(maxLinesPerReservation)+" lines at a time")
	}
