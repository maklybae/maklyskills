...
// Two reservations that each fit on their own must not both succeed when the
// shelf can only pay for one of them.
func TestConcurrentReservationsDoNotOversell(t *testing.T) {
	...
	stock, err := core.NewService(store.NewMemory(core.Level{SKU: "monitor-arm", OnHand: onHand}))
	if err != nil {
		t.Fatalf("NewService() error = %v", err)
	}

	var wait sync.WaitGroup
	results := make(chan error, contests)
	for i := range contests {
		wait.Add(1)
		go func() {
			defer wait.Done()
			results <- stock.Reserve(context.Background(), core.ReserveRequest{
				OrderID: "ord_" + strconv.Itoa(i),
				Lines:   []core.Line{{SKU: "monitor-arm", Quantity: 1}},
			})
		}()
	}
	wait.Wait()
	...
	for err := range results {
		...
		default:
			t.Fatalf("Reserve() error = %v, want nil or %v", err, core.ErrOutOfStock)
		}
	}
	if accepted != onHand {
		t.Fatalf("accepted %d reservations, want %d", accepted, onHand)
	}

	level, err := stock.Level(context.Background(), "monitor-arm")
	if err != nil {
		t.Fatalf("Level() error = %v", err)
	}
	if level.Reserved != onHand {
		t.Fatalf("reserved = %d, want %d", level.Reserved, onHand)
	}
}
...
func TestReturn(t *testing.T) {
	...
	levels, err := stock.Return(context.Background(), core.ReturnRequest{OrderID: "ord_1", Lines: lines})
	if err != nil {
		t.Fatalf("Return() error = %v", err)
	}
	if len(levels) != 2 {
		t.Fatalf("Return() gave back %d levels, want 2", len(levels))
	}
	if levels[0].OnHand != 44 || levels[1].OnHand != 9 {
		t.Fatalf("levels = %+v, want the returned units added", levels)
	}
}

func TestReturnRejects(t *testing.T) {
	tests := []struct {
		name    string
		request core.ReturnRequest
	}{
		{name: "no order id", request: core.ReturnRequest{Lines: []core.Line{{SKU: "desk-lamp", Quantity: 1}}}},
		{name: "no lines", request: core.ReturnRequest{OrderID: "ord_1"}},
		{
			name:    "a line of nothing",
			request: core.ReturnRequest{OrderID: "ord_1", Lines: []core.Line{{SKU: "desk-lamp"}}},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := stock.Return(context.Background(), tt.request)
			if !errors.Is(err, core.ErrInvalidRequest) {
				t.Fatalf("Return() error = %v, want %v", err, core.ErrInvalidRequest)
			}
			assertUntouched(t, stock)
		})
	}
