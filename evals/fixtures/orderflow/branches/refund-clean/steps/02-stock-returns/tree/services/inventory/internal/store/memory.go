...
// Reservations are kept: they belong to orders, not to the export.
...
func (m *Memory) LoadSnapshot(path string) error {
	...
	for _, lines := range m.reservations {
...
func (m *Memory) Restock(_ context.Context, sku string, quantity int) (core.Level, error) {
	...
	return m.addOnHand(sku, quantity), nil
}

// ReturnLines books a whole return in one go: the shelf must not be half
// updated if the caller goes away in the middle of it.
func (m *Memory) ReturnLines(_ context.Context, lines []core.Line) ([]core.Level, error) {
	m.mu.Lock()
	defer m.mu.Unlock()

	levels := make([]core.Level, 0, len(lines))
	for _, line := range lines {
		levels = append(levels, m.addOnHand(line.SKU, line.Quantity))
	}
	return levels, nil
}

func (m *Memory) addOnHand(sku string, quantity int) core.Level {
	level, ok := m.levels[sku]
	if !ok {
