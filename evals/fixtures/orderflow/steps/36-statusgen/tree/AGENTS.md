...
Three rules, all of them load bearing. A review that finds one of them broken
...
## 3. `internal/generated` is regenerated, never edited

`services/orders/internal/generated` is written by `make generate` from the
sources under `tools/`. To change what is in it, change the generator or the
declarations it reads, run the target and commit the result. A hand edit
survives until the next run and then disappears in someone else's diff.
