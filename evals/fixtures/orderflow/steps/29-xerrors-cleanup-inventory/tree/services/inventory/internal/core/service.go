...
func (s *Service) Reserve(ctx context.Context, req ReserveRequest) error {
	...
	_, held, err := s.store.Reservation(ctx, req.OrderID)
	if err != nil {
		return xerrors.Wrapf(err, "look up the reservation of order %s", req.OrderID)
	}
	if held {
		return nil
	}

	heldLevels := make([]Level, 0, len(req.Lines))
	for _, line := range req.Lines {
		level, err := s.store.Level(ctx, line.SKU)
		if err != nil {
			return xerrors.Wrapf(err, "load stock of %s", line.SKU)
		}
		if level.Available() < line.Quantity {
			return xerrors.Wrapf(ErrOutOfStock, "reserve %d of %s, %d available",
				line.Quantity, line.SKU, level.Available())
		}
		level.Reserved += line.Quantity
		heldLevels = append(heldLevels, level)
	}

	for _, level := range heldLevels {
		if err := s.store.SetLevel(ctx, level); err != nil {
			return xerrors.Wrapf(err, "hold stock of %s", level.SKU)
		}
	}
	if err := s.store.SaveReservation(ctx, req.OrderID, req.Lines); err != nil {
		return xerrors.Wrapf(err, "save the reservation of order %s", req.OrderID)
	}
	return nil
}
...
func (s *Service) Release(ctx context.Context, orderID string) (bool, error) {
	...
	lines, held, err := s.store.Reservation(ctx, orderID)
	if err != nil {
		return false, xerrors.Wrapf(err, "look up the reservation of order %s", orderID)
	}
	if !held {
		return false, nil
	}
	...
	for _, line := range lines {
		level, err := s.store.Level(ctx, line.SKU)
		if err != nil {
			return false, xerrors.Wrapf(err, "load stock of %s", line.SKU)
		}
		level.Reserved = max(level.Reserved-line.Quantity, 0)
		if err := s.store.SetLevel(ctx, level); err != nil {
			return false, xerrors.Wrapf(err, "give back the stock of %s", level.SKU)
		}
	}

	if err := s.store.DeleteReservation(ctx, orderID); err != nil {
		return false, xerrors.Wrapf(err, "drop the reservation of order %s", orderID)
	}
	return true, nil
}
...
func (s *Service) Restock(ctx context.Context, sku string, quantity int) (Level, error) {
	...
	if err := s.store.SetLevel(ctx, level); err != nil {
		return Level{}, xerrors.Wrapf(err, "book %d of %s in", quantity, sku)
	}
	return level, nil
}
