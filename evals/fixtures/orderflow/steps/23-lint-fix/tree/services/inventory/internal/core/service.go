...
func (s *Service) Reserve(ctx context.Context, req ReserveRequest) error {
	...
	if err != nil {
		return fmt.Errorf("look up the reservation of order %s: %w", req.OrderID, err)
	}
	if held {
		return nil
	}
	...
	for _, level := range heldLevels {
		if err := s.store.SetLevel(ctx, level); err != nil {
			return fmt.Errorf("hold stock of %s: %w", level.SKU, err)
		}
	}
...
func (s *Service) Release(ctx context.Context, orderID string) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("look up the reservation of order %s: %w", orderID, err)
	}
	if !held {
		return false, nil
	}
	...
	for _, line := range lines {
		...
		if err := s.store.SetLevel(ctx, level); err != nil {
			return false, fmt.Errorf("give back the stock of %s: %w", level.SKU, err)
		}
	}
...
func (s *Service) Restock(ctx context.Context, sku string, quantity int) (Level, error) {
	...
	if err := s.store.SetLevel(ctx, level); err != nil {
		return Level{}, fmt.Errorf("book %d of %s in: %w", quantity, sku, err)
	}
	return level, nil
}
