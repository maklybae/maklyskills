...
func (m *Memory) List(_ context.Context, filter core.Filter) ([]core.Order, error) {
	...
	return selectOrders(m.orders, filter), nil
}

// selectOrders is shared with the file backend so both answer a filter the same
// way, down to the order of the result.
func selectOrders(orders map[string]core.Order, filter core.Filter) []core.Order {
	selected := make([]core.Order, 0, len(orders))
