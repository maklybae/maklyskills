	...
	tests := []struct {
		...
		},
		{
			...
			run: func(t *testing.T, orders core.Store, order core.Order) {
				...
				stored.Status = core.StatusPaid
				if err := orders.Update(context.Background(), stored); err != nil {
...
// A write built on a copy the store has moved past is refused; the caller has
// to load the order again and redo its change.
func TestUpdateRefusesAStaleOrder(t *testing.T) {
	orders := newStore(t)
	if err := orders.Create(context.Background(), orderFor("ord_1", "cust-17", core.StatusPending, placedAt)); err != nil {
		t.Fatalf("Create() error = %v", err)
	}

	stale, err := orders.Get(context.Background(), "ord_1")
	if err != nil {
		t.Fatalf("Get() error = %v", err)
	}

	fresh := stale
	fresh.Status = core.StatusPaid
	if err := orders.Update(context.Background(), fresh); err != nil {
		t.Fatalf("Update() error = %v", err)
	}

	stale.Status = core.StatusShipped
	err = orders.Update(context.Background(), stale)
	if !errors.Is(err, core.ErrVersionConflict) {
		t.Fatalf("Update() error = %v, want %v", err, core.ErrVersionConflict)
	}
}
